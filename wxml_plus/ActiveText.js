...
class ActiveText {
    ...
    onChange(editor, resetCache) {
        ...
        if (languageId === 'wxml') {
            const _a = this.config.activeColor, tag = __rest(_a, ["color"]);
            tag['color'] = {};
            for (let i in tag) {
    ...
    decorateWxml(editor) {
        ...
        for (let i in tag) {
            ...
            let decorationType = vscode_1.window.createTextEditorDecorationType(Object.assign({}, { color: tag[i] || this.config.activeColor.color }));
